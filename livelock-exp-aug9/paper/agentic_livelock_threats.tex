...
\item We empirically show that adversarial delegation raises liveness failure rates from 5.47\% to 42.35\% across five open-source LLMs and two delegation types (a sixth model, Qwen3-4B, is evaluated but excluded from pooled statistics as a null control; see Section~\ref{sec:evaluating-attacks}), with AILD reaching up to 100 percentage points under controller-enforced policies.

\item We design and evaluate Delegation-based Moving Target Defense (D-MTD), which [FILL IN: same channel-qualified framing as the abstract — zero AILD on the URL/tool channel, 10-17.5\% residual AILD on the RAG channel in the two enforced regimes] where budget-cap and early-abort baselines fail.
\end{itemize}
...
Mistral & 7B & Open Source & Attack only$^\ddagger$ \\
...
$^\ddagger$ Defense runs incomplete at submission time. \quad
...
\midrule
Baseline        & 7.12 (800) & 16.93 (774) & 9.81 \\
Prompt-only     & 6.62 (800) & 24.27 (783) & 17.65 \\
Controller-only & 6.25 (480) & 79.11 (426) & 72.86 \\
Conservative    & \textbf{0.00} (479) & 83.48 (442) & 83.48 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:condition} aggregates liveness failure across the five pooled models and both delegation channels. Under benign delegation, the overall liveness failure rate is 5.47\% ($N=2559$; 95\% Wilson CI [4.65, 6.42]), confirming that the base task and delegation loop are mostly well-behaved, with the caveat on benign-side determinism noted in Section~\ref{sec:evaluating-attacks} above. Under adversarial delegation, this rises to 42.35\% ($N=2425$; CI [40.40, 44.33]), and the hit-budget rate tracks almost identically at 42.56\%. The near-perfect correspondence between hitting the budget and liveness failure indicates that agents which exhaust the delegation budget almost never recover: the adversarial channel leaves no residual high-confidence signal for the agent to act on. Table~\ref{tab:liveness_analysis} further reveals the policy dependence of this failure: benign liveness failure rates stay under 7.2\% across all regimes except Controller-only's 6.25\% and are exactly 0\% under Conservative, whereas adversarial failure rates stratify sharply by controller enforcement. Regimes with $\gamma=0$ (Baseline, Prompt-only) reach 16.93\% and 24.27\% adversarial LF respectively, while regimes with $\gamma=1$ (Controller-only, Conservative) reach 79.11\% and 83.48\%. This gap between enforced and unenforced regimes is the central finding of the attack evaluation: adversarial delegation alone is insufficient to induce reliable livelock; [FILL IN: your own framing of how large/decisive this enforced-vs-unenforced gap is — our earlier draft called it "three-fold" and that arithmetic no longer holds at roughly 4.5x to 5x on the current pooled numbers, so restate the comparison the way you want it].
...
\midrule
benign & 2559 & 5.47 & 5.47 \\
adversarial & 2425 & 42.56 & 42.35 \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:liveness_degradation} shows AILD across tool-based/URL and RAG-based delegation for each operational regime. Both channel types exhibit the same qualitative pattern: low AILD under $\gamma=0$ regimes and a sharp increase under $\gamma=1$ regimes. The URL channel currently has attack data for five models (Qwen2.5-7B/14B, Llama3.1-8B, Mistral-7B, Gemma3-12B) while the RAG channel currently has attack data for three (Qwen2.5-7B, Llama3.1-8B, Gemma3-12B; see Table~\ref{tab:llm_models}), so a same-model, matched comparison restricted to the three models present in both channels is necessary before comparing channels directly. On that matched three-model comparison, the direction reverses between the unenforced and enforced regimes: RAG shows higher AILD under Baseline (19.34\% vs.\ 5.48\%) and Prompt-only (19.33\% vs.\ 13.66\%), but URL shows higher AILD under Controller-only (64.47\% vs.\ 62.91\%, roughly tied) and Conservative (85.00\% vs.\ 71.34\%). This is the opposite ranking from our earlier draft's claim that RAG "consistently achieves higher AILD... across all regimes, reaching approximately 80\%," which was based on unequal model coverage per channel rather than a matched comparison. [FILL IN: this needs a real explanation, not a restated hypothesis — go back to the qwq7b_mistral URL-defense task and the pending 2B RAG Main Qwen-14B run on the pending-tasks list, decide whether to (a) qualify this as a preliminary three-model comparison, (b) hold this claim entirely until Qwen2.5-14B and Mistral-7B have RAG-channel attack data, or (c) drop the cross-channel ranking claim from this draft and keep only the "same qualitative pattern" statement]. Despite the reversal in channel ranking, the underlying attack mechanism — sustaining epistemic ambiguity through schema-valid, non-convergent responses — operates identically across both channel types.
...
\subsubsection{D-MTD most consistently restores liveness, with a channel-dependent gap}

Across the evaluated models and operational regimes,
D-MTD achieves the lowest observed AILD values of the three defenses in every cell of Table~\ref{tab:defense-results-url} and Table~\ref{tab:defense-results-rag}. On the tool-based/URL channel (Table~\ref{tab:defense-results-url}), D-MTD reaches exactly 0.0\% AILD in every Qwen2.5-14B and Qwen2.5-7B cell and in three of Llama3.1-8B's four cells. On the RAG channel (Table~\ref{tab:defense-results-rag}), D-MTD reaches 0.0\% under Baseline and Prompt-only but only reduces AILD to 10.0-17.5\% under Controller-only and Conservative — it does not reach zero on either evaluated RAG-channel model in the two enforced regimes. Unlike budget caps
or static stopping rules, D-MTD dynamically changes the
effective delegation surface during execution, directly targeting the structural prerequisite of the attack, but the RAG-channel enforced-regime residual shows this does not fully eliminate the vulnerability on every channel. [FILL IN: your own read on why the RAG channel leaves a residual where URL doesn't — e.g., whether it's the verifier-pool composition for RAG being smaller/different, retrieval-response verbosity making the progress-aware stagnation check less reliable, or something you'd want to check against the D-MTD implementation before claiming a mechanism].
...
[FILL IN: write your own closing characterization of the moving-target-defense result now that it is channel-qualified rather than uniform — how strongly to claim "particularly well-suited," and whether to foreground the URL-channel zero-AILD result or the RAG-channel residual as the more important message for a reviewer].
...
\centering
\caption{Defense Results (AILD Score), tool-based/URL channel. Nominal $N$=100 per cell (Baseline/Prompt-only) or $N$=60 (Controller-only/Conservative); Qwen2.5-14B and Llama3.1-8B have occasional incomplete-trial shortfalls down to $N$=40 in a small number of cells. Mistral-7B is omitted: its URL-channel defense run is not yet complete (Table~\ref{tab:llm_models}). Exact per-cell $N$ is in the released gold-standard CSVs.}
