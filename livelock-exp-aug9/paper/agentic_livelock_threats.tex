...
& Prompt-only & 36.0 & 49.0 & -40.0$^\dagger$ & -3.0$^\dagger$ \\
...
\textbf{Model} & \textbf{Regime} & \textbf{No Defense} & \textbf{Budget cap} & \textbf{Early-abort} & \textbf{D-MTD} \\
\midrule
\multirow{4}{*}{Qwen2.5-14B} & Baseline & 2.5 & 37.5 & 0.0 & 0.0 \\
& Prompt-only & 2.5 & 42.5 & 0.0 & 0.0 \\
& Controller-only & 100.0 & 100.0 & 12.5 & 10.0 \\
& Conservative & 100.0 & 100.0 & 5.0 & 17.5 \\
\midrule
\multirow{4}{*}{Qwen2.5-7B} & Baseline & 5.0 & 25.0 & 0.0 & 0.0 \\
& Prompt-only & 0.0 & 30.0 & 0.0 & 0.0 \\
& Controller-only & 100.0 & 100.0 & 12.5 & 12.5 \\
& Conservative & 100.0 & 100.0 & 17.5 & 10.0 \\
\bottomrule
\end{tabular}
\end{table}
%
...
Our evaluation uses simple deterministic tasks — arithmetic and string-counting on the tool-based/URL channel, two fixed policy-document questions on the RAG channel — that are intentionally chosen to isolate delegation and completion logic from task difficulty. Each channel currently uses only two distinct task instances under greedy decoding, which makes most benign-condition trials deterministic replicates rather than independent draws (Section~\ref{sec:evaluating-attacks}); broadening task diversity before a final release is planned but not yet done. Model coverage is also currently uneven across attack vs.\ defense evaluation: five models have attack data on both channels (Table~\ref{tab:llm_models}), but only four have URL-channel defense data and only two have RAG-channel defense data. Gemma3-12B has no defense data on either channel by design (Table~\ref{tab:llm_models}), and RAG-channel defense for Mistral-7B and Llama3.1-8B has not been run and is not currently planned. [FILL IN: decide how prominently to foreground this coverage gap in the limitations paragraph versus treating it as resolved by the per-table footnotes above]. Whether the same livelock dynamics emerge under complex, real-world agentic tasks (e.g., software development, web navigation, multi-step reasoning) remains an open question. Additionally, we evaluate a black-box outsider attacker who does not adapt to the deployed defense: an adaptive attacker who probes D-MTD's channel rotation behavior and adjusts confidence values to counter threshold perturbation could potentially reduce D-MTD's effectiveness. Our evaluation is also limited to open-source models; evaluating proprietary systems such as GPT-4 or Claude would reveal whether architectural or RLHF differences alter livelock susceptibility. All statistics reported in this paper are descriptive: point estimates with Wilson 95\% confidence intervals, not inferential hypothesis tests. Two specific gaps remain open and are flagged here rather than silently assumed away. First, no significance test has been run on any benign-vs-adversarial or defense-vs-no-defense comparison in this paper. When this is run, it should use Fisher's exact test (the normal-approximation two-proportion $z$-test is unreliable at the many 0\% and 100\% cells in this data, per the interval-estimation discussion in Section~\ref{sec:evaluating-attacks}), with Holm-Bonferroni correction applied within two separate families: all benign-vs-adversarial comparisons in Tables~\ref{tab:liveness_analysis}-\ref{tab:per_model_aild} as one family, and all defense-vs-no-defense comparisons in Tables~\ref{tab:defense-results-url}-\ref{tab:defense-results-rag} as a second, separately-corrected family, since correcting both together would apply an unnecessarily conservative penalty to each. Until this is run, claims such as Gemma3-12B's sub-saturation AILD ceiling (Table~\ref{tab:per_model_aild}) or Mistral-7B's D-MTD residual (Table~\ref{tab:defense-results-url}) being "real" rest on non-overlapping or barely-overlapping confidence intervals, which is suggestive but not the same as a tested, corrected significance claim. Second, we have not yet fit a mixed-effects or other repeated-measures model (e.g., $\mathrm{LF} \sim \mathrm{condition}*\mathrm{regime} + \mathrm{channel\_type} + (1\mid\mathrm{model})$) across the combined attack dataset; the pooled percentages and Wilson confidence intervals reported here treat trials within a cell as independent draws, which Section~\ref{sec:evaluating-attacks} shows is only approximately true on the benign side given the two-task design, and treat each model as an independent replicate rather than modeling model identity as a random effect. Both gaps are scoping decisions for this draft, not oversights we are unaware of, and both should be closed before treating the per-model heterogeneity (Table~\ref{tab:per_model_aild}) or the defense residuals (Tables~\ref{tab:defense-results-url}-\ref{tab:defense-results-rag}) as statistically confirmed rather than descriptively suggestive. Finally, while D-MTD provides strong empirical liveness restoration for 2 of 4 URL-channel models, formally verifying liveness guarantees under bounded adversarial pool compositions remains future work, as does integration with runtime monitoring frameworks \cite{koohestani2025agentguard} to enable continuous liveness assurance in deployed agentic systems, and as does closing the Mistral-7B URL-channel residual and the RAG-channel enforced-regime residual identified in Section~\ref{sec:evaluating-defenses}.
